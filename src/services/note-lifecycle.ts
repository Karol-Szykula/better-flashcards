...
/**
 * What the import wizard may do with a note. Import only pulls from Anki, so a
 * note whose vault side is newer is left to Sync unless the user forces this
 * one note; a note the wizard has no business touching resolves to undefined.
 * "Anki wins" takes Anki's version wherever the default would refuse: a newer
 * vault note is overwritten, a deleted file is resurrected. Where the default
 * already writes Anki's version, forcing changes nothing.
 */
export function importActFor(
  status: NoteLifecycleStatus,
  isForced = false,
): NoteLifecycleEvent | undefined {
  if (isForced) {
    const forcedAct = forcedActFor(status);
    if (forcedAct !== undefined) {
      return forcedAct;
    }
  }
  switch (status) {
    case "ankiOnly.neverImported":
      return "IMPORT";
    case "synced.clean":
      return "CHECK";
    ...
    case "synced.diverged":
      return undefined;
    case "linked.unenrolled":
    case "vaultOnly.unenrolled":
      return "ENROLL";
    default:
      return undefined;
  }
...
/**
 * What the export wizard may do with a note. It creates notes Anki does not
 * have and pushes what changed in Obsidian. A note edited on both sides or
 * deleted in Anki waits for the user: "Obsidian wins" forces it, and without
 * that force it is left to Sync, never resolved by a guess.
 */
export function exportActFor(
  status: NoteLifecycleStatus,
  isForced = false,
): NoteLifecycleEvent | undefined {
  if (isForced) {
    const forcedAct = forcedExportActFor(status);
    if (forcedAct !== undefined) {
      return forcedAct;
    }
  }
  switch (status) {
    case "vaultOnly.unexported":
      return "EXPORT";
    case "linked.unenrolled":
    case "vaultOnly.unenrolled":
      return "ENROLL";
...
function forcedExportActFor(
  status: NoteLifecycleStatus,
): NoteLifecycleEvent | undefined {
  switch (status) {
    case "vaultOnly.ankiDeleted":
      return "EXPORT";
    case "synced.ankiNewer":
    case "synced.diverged":
      return "FORCE_PUSH";
...
/** What Sync may do with a note: newest version wins in both directions. */
export function syncActFor(
  status: NoteLifecycleStatus,
): NoteLifecycleEvent | undefined {
  switch (status) {
    case "synced.clean":
