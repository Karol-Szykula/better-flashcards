...
describe("parseYamlNotes", () => {
  test("given content with one form block when parsed then returns the card", async () => {
    ...
    // when
    const parsed = parseYamlNotes(content, jsonEngine);

    // then
    expect(parsed).toEqual([
      { back: "A", extra: {}, front: "Q", id: 7, model: "Basic", tags: "math" },
    ]);
  });
...
describe("computeContentHash", () => {
  test("given identical content when hashed then returns the same hex digest", async () => {
    ...
    // when
    const first = await computeContentHash(front, back, tags, model);
    const second = await computeContentHash(front, back, tags, model);
  ...
  test("given changed back when hashed then returns a different digest", async () => {
    ...
    // when
    const original = await computeContentHash(front, "4", tags, model);
    const changed = await computeContentHash(front, "5", tags, model);

    // then
    expect(changed).not.toBe(original);
  });

  test("given changed model when hashed then returns a different digest", async () => {
    // given
    const front = "Q";
    const back = "A";
    const tags = "";
    ...
    // when
    const basic = await computeContentHash(front, back, tags, "Basic");
    const cloze = await computeContentHash(front, back, tags, "Cloze");

    // then
    expect(cloze).not.toBe(basic);
  });
});

describe("serializeYamlNote", () => {
  test("given a card when serialized then wraps it in a fenced block", async () => {
    // given
    const card = yamlCard({ extra: {}, id: undefined });

    // when
    const serialized = serializeYamlNote(card, jsonEngine);

    // then
    expect(serialized).toBe(
      '```note-form\n{"front":"What is 2+2?","back":"4","model":"Basic","tags":"math"}\n```',
    );
  });

  test("given a cloze card when serialized then writes native cloze keys", async () => {
    // given
    const card = yamlCard({
      back: "Capital",
      extra: {},
    ...
    // then
    expect(serialized).toBe(
      '```note-form\n{"text":"Paris is {{c1::France}}","back_extra":"Capital","model":"Cloze","tags":"geo","id":9}\n```',
    );
  });
});
...
describe("readYamlNotes", () => {
  test("given a vault file with a form block when read then returns the card", async () => {
    ...
    // when
    const cards = await readYamlNotes(vault, file, jsonEngine);
